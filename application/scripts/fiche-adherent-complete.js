import { listAdherents, listInscriptions } from "../firebase/firebase-db.js";
...
function champ(donnees, mots) {
    ...
    const cleDirecte = Object.keys(donnees).find(cle => motsNormalises.every(mot => nettoyer(cle).includes(mot)));
    if (cleDirecte && donnees[cleDirecte] !== undefined && donnees[cleDirecte] !== null) return donnees[cleDirecte];
    const customFields = Array.isArray(donnees.customFields) ? donnees.customFields : [];
    const trouve = customFields.find(item => motsNormalises.every(mot => nettoyer(item?.name).includes(mot)));
...
async function initialiserFicheComplete() {
    try {
        [adherents, inscriptions] = await Promise.all([listAdherents(), listInscriptions()]);
        adherent = adherents.find(item => String(item.numeroAdherent) === String(numero) || String(item.id) === String(numero)) || null;
        historique = inscriptions.filter(item => String(item.numeroAdherent) === String(adherent?.numeroAdherent || numero));
        derniereInscription = historique[historique.length - 1] || null;
        donnees = derniereInscription?.donneesHelloAsso || derniereInscription?.donnees || {};
